...
const toggleTheme = () => {
  ...
  // 如果音乐已启用，自动切换音乐并保持播放状态
  if (appStore.isMusicEnabled) {
    const wasPlaying = appStore.isMusicPlaying
    const newTrack = appStore.isDarkMode ? 'night' : 'day'
    
    console.log('主题切换音乐同步:', {
      wasPlaying,
      newTrack,
    ...
    // 设置新曲目
    appStore.currentMusicTrack = newTrack
